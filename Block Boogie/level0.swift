...
//  Block Boogie
//
//  Created by Patrick Sweeney on 7/23/16.
...
class level0: SKScene, SKPhysicsContactDelegate
{
    ...
    var introtext:SKLabelNode = SKLabelNode()
    ...
    var yellow:SKSpriteNode = SKSpriteNode()
    //------------------------------------------------
    ...
    override func didMove(to view: SKView)
    {
        ...
        //yellow block
        if let yb:SKSpriteNode = self.childNode(withName: "yellowblock") as? SKSpriteNode
        {
            yellow = yb
            
        }
        //introTEXT
        if let it:SKLabelNode = self.childNode(withName: "introText") as? SKLabelNode
        {
            introtext = it
            introtext.run(SKAction.sequence([ SKAction.wait(forDuration: 2.5), SKAction.run({ self.introtext.text = "WELCOME TO BLOCK BOOGIE"}),SKAction.wait(forDuration: 3), SKAction.run({ self.introtext.text = "THIS IS A SKILL BASED PLATFORMER"}),SKAction.wait(forDuration: 3), SKAction.run({ self.introtext.text = "YOU PLAY AS THAT LITTLE BLUE SQUARE"}),SKAction.wait(forDuration: 4), SKAction.run({ self.introtext.text = "PRESS 'OPTIONS' THEN 'TOGGLE UI'"}),SKAction.wait(forDuration: 5), SKAction.run({ self.introtext.text = "TO HIDE THE CONTROLS"}),SKAction.wait(forDuration: 3), SKAction.run({ self.introtext.text = "THREE BUTTONS FOR LEFT,RIGHT,JUMP"}),SKAction.wait(forDuration: 3), SKAction.run({ self.introtext.text = "THANKS FOR PLAYING AND HAVE FUN"}), SKAction.run({ self.yellow.run(SKAction.moveBy(x: 0, y: 150.0, duration: 4.0)) }) ]))
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level0")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block0")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
