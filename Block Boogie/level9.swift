...
//  Block Boogie
//
//  Created by Patrick Sweeney on 6/19/16.
...
class level9: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)
    {
        ...
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock23") as? SKSpriteNode
        {
        ...
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock24") as? SKSpriteNode
        {
    ...
    func didBegin(_ contact: SKPhysicsContact)
    {
        ...
        if(((contact.bodyA.categoryBitMask == playerCategory) && (contact.bodyB.categoryBitMask == chaseblockCategory)) || ((contact.bodyA.categoryBitMask == chaseblockCategory ) && (contact.bodyB.categoryBitMask == playerCategory)))
        {
            ...
            {
                chaseOngoing = true
                chaseblock.run(SKAction.moveBy(x: 0, y: 7800, duration: 60))
            }
        }
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level9")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block9")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
