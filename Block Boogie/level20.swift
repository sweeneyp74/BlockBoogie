...
//  Block Boogie
//
//  Created by Patrick Sweeney on 8/24/16.
...
class level20: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)
    {
        ...
        {
            CheckPoint = cp
            CheckPoint.CheckPointPosition = CGPoint(x: CheckPoint.position.x, y: CheckPoint.position.y + 150)
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level20")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block20")
            UserDefaults.standard.synchronize()
        }
