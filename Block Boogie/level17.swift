...
//  Block Boogie
//
//  Created by Patrick Sweeney on 7/22/16.
...
class level17: SKScene, SKPhysicsContactDelegate
{
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level17")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block17")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
