...
//  Block Boogie
//
//  Created by Patrick Sweeney on 6/26/16.
...
class level12: SKScene, SKPhysicsContactDelegate
{
    ...
    let redblockmove: SKAction = SKAction.moveBy(x: 950, y: 0, duration: 5)
    let redblockmove2: SKAction = SKAction.moveBy(x: 850, y: 0, duration: 1)
    //------------------------------------------------
    override func didMove(to view: SKView)
    {
        ...
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock3") as? SKSpriteNode
        {
            ...
            redBlock3.run(SKAction.repeatForever(SKAction.sequence([redblockmove, redblockmove.reversed()])))
        }
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock4") as? SKSpriteNode
        {
            ...
            redBlock4.run(SKAction.repeatForever(SKAction.sequence([redblockmove, redblockmove.reversed()])))
        }
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock5") as? SKSpriteNode
        {
            ...
            redBlock5.run(SKAction.repeatForever(SKAction.sequence([redblockmove, redblockmove.reversed()])))
        }
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock6") as? SKSpriteNode
        {
            ...
            redBlock6.run(SKAction.repeatForever(SKAction.sequence([redblockmove2, redblockmove2.reversed()])))
        }
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock7") as? SKSpriteNode
        {
            ...
            redBlock7.run(SKAction.repeatForever(SKAction.sequence([redblockmove2.reversed(), redblockmove2])))
        }
        ...
        if let someBlackBlock:SKSpriteNode = self.childNode(withName: "blackblock1") as? SKSpriteNode
        {
            ...
            blackblock1.physicsBody!.categoryBitMask = redblockCategory
            blackblock1.physicsBody!.contactTestBitMask = redblockCategory | playerCategory
            
            let moveplatform1: SKAction = SKAction.moveBy(x: 0, y: 400, duration: TimeInterval(2.0))
            blackblock1.run(SKAction.repeatForever(SKAction.sequence([moveplatform1, moveplatform1.reversed() ])))
        }
        if let someBlackBlock2:SKSpriteNode = self.childNode(withName: "blackblock2") as? SKSpriteNode
        {
            blackblock2 = someBlackBlock2
            blackblock2.physicsBody!.categoryBitMask = redblockCategory
            blackblock2.physicsBody!.contactTestBitMask = redblockCategory | playerCategory
            
            let moveplatform2: SKAction = SKAction.moveBy(x: 0, y: 400, duration: TimeInterval(2.0))
            blackblock2.run(SKAction.repeatForever(SKAction.sequence([moveplatform2,moveplatform2.reversed() ])))
        }
    ...
    func endOfLevel()
    {
        let move: SKAction = SKAction.moveBy(x: 0, y: 300, duration: TimeInterval(1.5))
        endblock1.run(move)
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level12")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block12")
            UserDefaults.standard.synchronize()
        }
    ...
    func moveCheckPoint()
    {
        if(isCheckPointActive == false)
        {
            ...
            let movecp: SKAction = SKAction.moveBy(x: 0, y: -300, duration: 3)
