...
//  Block Boogie
//
//  Created by Patrick Sweeney on 6/25/16.
...
class level11: SKScene, SKPhysicsContactDelegate
{
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level11")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block11")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
