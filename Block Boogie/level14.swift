...
//  Block Boogie
//
//  Created by Patrick Sweeney on 6/29/16.
...
class level14: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)
    {
        ...
        {
            CheckPoint = cp
            CheckPoint.CheckPointPosition = CGPoint(x: CheckPoint.position.x - 100, y: CheckPoint.position.y)
        ...
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock1") as? SKSpriteNode
        {
            ...
            redBlock1.run(SKAction.repeatForever(SKAction.sequence([redblockmove, redblockmove2, redblockmove.reversed(), redblockmove2.reversed()])))
        }
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redblock2") as? SKSpriteNode
        {
            ...
            redBlock2.run(SKAction.repeatForever(SKAction.sequence([redblockmove.reversed(), redblockmove2.reversed(), redblockmove, redblockmove2 ])))
        }
        ...
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redspin1") as? SKSpriteNode
        {
            ...
            let redSpinAction = SKAction.repeatForever( SKAction.rotate(byAngle: 15, duration: 10))
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level14")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block14")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
    ...
    func moveCheckPoint()
    {
        if(isCheckPointActive == false)
        {
            ...
            let movecp: SKAction = SKAction.moveBy(x: 0, y: 750, duration: 1)
    ...
    func resetChase()
    {
        redBlock6.position = CGPoint(x: 1347.454, y: 7130.174)
        chaseOngoing = false
    }
