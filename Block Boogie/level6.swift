...
class level6: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)
    {
        ...
        {
            CheckPoint = cp
            CheckPoint.CheckPointPosition = CGPoint(x: CheckPoint.position.x - 200, y: CheckPoint.position.y)
        ...
        if let someRedBlock:SKSpriteNode = self.childNode(withName: "redspin1") as? SKSpriteNode
        {
            ...
            let redSpinAction = SKAction.repeatForever( SKAction.rotate(byAngle: 3.1416, duration: 22))
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level6")
        UserDefaults.standard.synchronize()

        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block6")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
    ...
    func moveCheckPoint()
    {
        if(isCheckPointActive == false)
        {
            ...
            let movecp: SKAction = SKAction.moveBy(x: 0, y: 1025, duration: 1)
