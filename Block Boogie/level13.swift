...
//  Block Boogie
//
//  Created by Patrick Sweeney on 6/28/16.
...
class level13: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMove(to view: SKView)
    {
        ...
        {
            CheckPoint = cp
            CheckPoint.CheckPointPosition = CGPoint(x: CheckPoint.position.x, y: CheckPoint.position.y + 200)
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level13")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block13")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
