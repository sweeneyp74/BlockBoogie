...
class level2: SKScene, SKPhysicsContactDelegate
{
    ...
    let redblockmove: SKAction = SKAction.moveByX(950, y: 0, duration: 5)
    let redblockmove2: SKAction = SKAction.moveByX(850, y: 0, duration: 1)
    //------------------------------------------------
    override func didMoveToView(view: SKView)
    {
        ...
        if let someRedBlock:SKSpriteNode = self.childNodeWithName("redblock6") as? SKSpriteNode
        {
            ...
            redBlock6.runAction(SKAction.repeatActionForever(SKAction.sequence([redblockmove2, redblockmove2.reversedAction()])))
        }
        if let someRedBlock:SKSpriteNode = self.childNodeWithName("redblock7") as? SKSpriteNode
        {
            ...
            redBlock7.runAction(SKAction.repeatActionForever(SKAction.sequence([redblockmove2.reversedAction(), redblockmove2])))
        }
        ...
        if let someBlackBlock:SKSpriteNode = self.childNodeWithName("blackblock1") as? SKSpriteNode
        {
            ...
            let moveplatform1: SKAction = SKAction.moveByX(0, y: 400, duration: NSTimeInterval(2.0))
            blackblock1.runAction(SKAction.repeatActionForever(SKAction.sequence([moveplatform1, moveplatform1.reversedAction() ])))
        }
        if let someBlackBlock2:SKSpriteNode = self.childNodeWithName("blackblock2") as? SKSpriteNode
        {
            ...
            let moveplatform2: SKAction = SKAction.moveByX(0, y: 400, duration: NSTimeInterval(2.0))
            blackblock2.runAction(SKAction.repeatActionForever(SKAction.sequence([moveplatform2,moveplatform2.reversedAction() ])))
        }
    ...
    func endOfLevel()
    {
        let move: SKAction = SKAction.moveByX(0, y: 300, duration: NSTimeInterval(1.5))
        ...
        let levelbool = true
        NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"level2")
        NSUserDefaults.standardUserDefaults().synchronize()
        
        if(collectibleFound == true)
        {
            NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"block2")
            NSUserDefaults.standardUserDefaults().synchronize()
        }
    ...
    func moveCheckPoint()
    {
        if(isCheckPointActive == false)
        {
            ...
            let movecp: SKAction = SKAction.moveByX(0, y: -300, duration: 3)
