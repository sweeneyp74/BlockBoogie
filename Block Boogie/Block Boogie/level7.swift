...
//  Block Boogie
//
//  Created by Patrick Sweeney on 6/1/16.
...
class level7: SKScene, SKPhysicsContactDelegate
{
    ...
    func didBeginContact(contact: SKPhysicsContact)
    {
        ...
        if(((contact.bodyA.categoryBitMask == playerCategory) && (contact.bodyB.categoryBitMask == chaseblockCategory)) || ((contact.bodyA.categoryBitMask == chaseblockCategory ) && (contact.bodyB.categoryBitMask == playerCategory)))
        {
            ...
            {
                chaseOngoing = true
                redBlock6.runAction(SKAction.moveByX(8000, y: 0, duration: 60))
            }
        }
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"level7")
        NSUserDefaults.standardUserDefaults().synchronize()
        
        if(collectibleFound == true)
        {
            NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"block7")
            NSUserDefaults.standardUserDefaults().synchronize()
        }
    }
    func goBackToHub()
