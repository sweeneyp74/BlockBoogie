...
//  Block Boogie
//
//  Created by Patrick Sweeney on 5/31/16.
...
class level6: SKScene, SKPhysicsContactDelegate
{
    ...
    override func didMoveToView(view: SKView)
    {
        ...
        {
            CheckPoint = cp
            CheckPoint.CheckPointPosition = CGPointMake(CheckPoint.position.x - 200, CheckPoint.position.y)
        ...
        if let someRedBlock:SKSpriteNode = self.childNodeWithName("redspin1") as? SKSpriteNode
        {
            ...
            let redSpinAction = SKAction.repeatActionForever( SKAction.rotateByAngle(3.1416, duration: 20))
    ...
    func didBeginContact(contact: SKPhysicsContact)
    {
        ...
        {
            print("bounce")
            thePlayer.physicsBody!.applyImpulse(CGVectorMake(0, 200))
        }
        //endblock
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"level6")
        NSUserDefaults.standardUserDefaults().synchronize()

        if(collectibleFound == true)
        {
            NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"block6")
            NSUserDefaults.standardUserDefaults().synchronize()
        }
    }
    func goBackToHub()
    ...
    func moveCheckPoint()
    {
        if(isCheckPointActive == false)
        {
            ...
            let movecp: SKAction = SKAction.moveByX(0, y: 1025, duration: 1)
