...
//  Block Boogie
//
//  Created by Patrick Sweeney on 5/28/16.
...
class level4: SKScene, SKPhysicsContactDelegate
{
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"level4")
        NSUserDefaults.standardUserDefaults().synchronize()

        if(collectibleFound == true)
        {
            NSUserDefaults.standardUserDefaults().setObject(levelbool, forKey:"block4")
            NSUserDefaults.standardUserDefaults().synchronize()
        }
    }
    func goBackToHub()
