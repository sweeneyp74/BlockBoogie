...
//  Block Boogie
//
//  Created by Patrick Sweeney on 8/29/16.
...
class level21: SKScene, SKPhysicsContactDelegate
{
    ...
    func didBegin(_ contact: SKPhysicsContact)
    {
        ...
        if(((contact.bodyA.categoryBitMask == playerCategory) && (contact.bodyB.categoryBitMask == redblockCategory)) || ((contact.bodyA.categoryBitMask == redblockCategory ) && (contact.bodyB.categoryBitMask == playerCategory)))
        {
            ...
            deathLabel.text = "Honestly, I always kinda liked you"
    ...
    func endOfLevel()
    {
        ...
        endblock1.run(move)
        playerCamera.run(SKAction.scale(to: 1.00, duration: 3))
        run(SKAction.sequence([SKAction.wait(forDuration: 3.5), SKAction.run(goBackToHub)]))
        
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level21")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block21")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
    ...
    func moveCheckPoint()
    {
        if(isCheckPointActive == false)
        {
            ...
            let movecp: SKAction = SKAction.moveBy(x: 0, y: -160, duration: 3)
