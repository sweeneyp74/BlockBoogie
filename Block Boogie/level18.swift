...
//  Block Boogie
//
//  Created by Patrick Sweeney on 7/30/16.
...
class level18: SKScene, SKPhysicsContactDelegate
{
    ...
    func endOfLevel()
    {
        ...
        let levelbool = true
        UserDefaults.standard.set(levelbool, forKey:"level18")
        UserDefaults.standard.synchronize()
        
        if(collectibleFound == true)
        {
            UserDefaults.standard.set(levelbool, forKey:"block18")
            UserDefaults.standard.synchronize()
        }
    }
    func goBackToHub()
    ...
    func resetChase()
    {
        redBlock6.position = CGPoint(x: 1367.452, y: 7110.174)
        chaseOngoing = false
    }
